...
import { processAddPending, processDeletePendingExpense, processFulfillPendingExpense, processUpdatePendingExpenses } from "@/services/expenses-calculator";
...
export async function PATCH(request: NextRequest) {
	try {
		...
		if (!existingTable || !existingTable._id) return NextResponse.json({ error: 'No active table found' }, { status: 404 });
		const updatedTable = await processFulfillPendingExpense(pendingExpenseId, existingTable);
		await collection.updateOne({ _id: existingTable._id as ObjectId }, {
			$set: { pending: updatedTable.pending, totals: updatedTable.totals, lastModified: new Date().getTime() },
		});
		await client.close();

...
import { getCurrentExpenses, setCurrentExpenses, setPending } from "@/services/simple-expenses.service";
...
export async function addPendingExpense(currentTable_id, pendingData) {
  const { client, collection } = await setInitialValues();
  const table_id = convertToObjectId(currentTable_id);
  let updatedTable = setPending(pendingData);
  const res = await collection.findOneAndUpdate({ _id: table_id }, {
    $set: { pending: pendingData, totals: updatedTable.totals },

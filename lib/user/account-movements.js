...
import { getAccount, restartCumulativeMovements, setAccount, setExpenses, setIncomes, setRemaining, setSavingPots, setWithdrawals } from "@/services/account-movements.service";
...
export async function createCategories(categoryList) {
	...
	try {
		...
		if (!response) {
			await collection.insertOne(categoriesObj);
		} else {
			await collection.updateOne({ _id: response._id }, {
...
export async function updateIncomeList(incomeList, account) {
	...
	if (currentAccount) {
		...
		try {
			await collection.updateOne({ _id: currentAccount._id }, {
...
export async function closeAccountPeriod() {
	...
	try {
		...
		const closed_cumulative_section = restartCumulativeMovements(accountMovements.cumulative_section);
		let newData = { last_balance: accountMovements.remaining, cumulative_section: closed_cumulative_section, saving_pots: accountMovements.saving_pots };
		const recurrentData = await collection.findOne({ user_id: session.user.email });
		if (!recurrentData) {

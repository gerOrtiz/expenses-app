		...
		<Dialog size="sm"
			...
			<DialogHeader>
				<div className="flex w-full justify-between items-center">
					<Typography variant="h5" className="text-blue-800" id="date-range-dialog-label">
						{`Date range`}
					</Typography>
			...
			<DialogBody className="pt-0">
				<div className="flex w-full flex-col gap-3 p-1">
					<Typography color="blue-gray" variant="paragraph" id="date-range-dialog-description">
						{`Update pending expense details`}
					</Typography>
					<form onSubmit={handleSubmit(onSubmit)}>
						<div className="w-full flex gap-3">
							<div className="flex flex-col" >
								<label htmlFor="start-date" className="text-xs text-gray-800 font-semibold">{'Start date'}</label>
								<input id="start-date" type="date" className="formInput" {...register('startDate', { required: true })} />
							</div>
							<div className="flex flex-col" >
								<label htmlFor="end-date" className="text-xs text-gray-800 font-semibold">{'End date'}</label>
